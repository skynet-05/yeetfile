...
const BUFFER_SIZE int = 16384 // 16kb
...
func main() {
	...
	for idx < len(file) {
		...
		encryptedChunk := encrypt(key, file[idx:idx+chunkSize])

		_, err = output.Write(encryptedChunk)
		if err != nil {
...
func encrypt(key [KEY_SIZE]byte, data []byte) []byte {
